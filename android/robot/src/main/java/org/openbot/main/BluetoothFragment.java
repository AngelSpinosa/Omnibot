...
public class BluetoothFragment extends Fragment {
    ...
    vehicle.setBleAdapter(
            ...
            new CommonRecyclerViewAdapter.OnItemClickListener() {
              ...
              public void onItemClick(View itemView, int position) {
                // VERIFICACIÓN DE SEGURIDAD ANTES DE DETENER SCAN
                // Como BluetoothManager ahora maneja la excepción internamente, esto es seguro de llamar.
                vehicle.stopScan();

                ProgressBar pb = getView().findViewById(R.id.progress_bar);
                View tv = getView().findViewById(R.id.btn_refresh);
                if(pb != null) pb.setVisibility(View.INVISIBLE);
                if(tv != null) tv.setVisibility(View.VISIBLE);

                BluetoothDevice device = vehicle.getDeviceList().get(position);

                if (vehicle.getBleDevice() == null
                        || vehicle.getBleDevice().getAddress().equals(device.getAddress())) {
  ...
  private void checkPermission(boolean grantAll) {
    ...
    if (grantAll) {
      ...
      if (adapter != null && !adapter.isEnabled()) {
        // Opcional: Solicitar encendido de BT
      }
  ...
  private void startScan() {
    ...
    // Llamada segura a Vehicle -> BluetoothManager
    vehicle.startScan();
  ...
  public void onDestroy() {
    super.onDestroy();
    if(vehicle != null) vehicle.stopScan();
  }
}