...
// Intenta conectarte al dispositivo serial
                      ...
                      if (FormatUtils.isNumeric(body)) {
                        ...
                        runOnUiThread(
                            () -> {
                              ...
                              System.out.println(String.format(Locale.US, "%3.0f cm", vehicle.getSonarReading()));
  ...
  /**METODO PARA CREAR LAS DIMENSIONES DE LA IMAGEN CAPTADAS POR LA CAMARA**/
  @Override
  ...
  //Recopilacion de datos para crear un JSON para intercambiar informacion
  ...
  protected void toggleConnection(boolean isChecked) {
    ...
    if (vehicle.usbEstaConectada()) {
      ...
      if (isChecked) {
        Toast.makeText(getContext(), "Por favor verifique la conexión USB.", Toast.LENGTH_SHORT).show();
      }
    }
  }
  ...
  */
  ...
  //CLASE PARA MANEJAR EL TIPO DE EVENTO SI ESTA CONECTADO MEDIANTE LA API
  private void handleControllerEvents() {
    ControllerToBotEventBus.subscribe(
        this.getClass().getSimpleName(),
        //Define un manejador de eventos que recibe un JSON, este JSON trae el comando a ejecutar
        event -> {
          JSONObject commandJsn = event;
          String commandType = "";
          //Registra un mensaje de depuracion.
          Log.d(null, "Got command from controller: " + commandJsn.toString());
          if (commandJsn.has("command")) {           //Verifica que el objeto JSON tiene un clave llamada "command"
            commandType = commandJsn.getString("command");  //Recupera el valor asociado a la clave "command" y la almacena en la variable commandTYPE
          } else if (commandJsn.has("driveCmd")) { //Si este objeto JSON no tiene una clave llamda "command"
            commandType = "DRIVE_CMD";      //se asgina a la variable commandTYPE un valor de "DRVIE_CMD"
          }
          switch (commandType) { //Dependiendo del comando que tenga la variable commandTYPE
            case "DRIVE_CMD":
            ...
            case "CONNECTED":
              ...
              // That is why we are not calling emit.send() here directly.
              BotToControllerEventBus.emitEvent(
  ...
  // Controlador de eventos del controlador
